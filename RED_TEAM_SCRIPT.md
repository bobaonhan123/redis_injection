...
In the Kubernetes demo we also add an optional lateral blast: the compromised web pod probes an internal service (`blast-target`) via SSRF before NetworkPolicy is applied.
...
Trong demo Kubernetes, mình bổ sung một bước blast lateral: web pod đã bị chiếm sẽ probe một service nội bộ (`blast-target`) qua SSRF trước khi áp NetworkPolicy.
...
Optional step four (K8s): the compromised web pod also sends a raw HTTP request via SSRF to the `blast-target` service to demonstrate lateral reachability before NetworkPolicy is enforced.
...
Bước bốn (tuỳ chọn, K8s): web pod bị chiếm gửi HTTP thô qua SSRF tới service `blast-target` để minh hoạ khả năng lateral trước khi bật NetworkPolicy.
...
We first show a pre-policy blast: the compromised web pod can reach `blast-target`. After NetworkPolicy is applied, the same probe is blocked.
...
Mình sẽ minh hoạ 2 trạng thái: trước khi áp policy thì có thể blast sang `blast-target`; sau khi áp policy thì probe bị chặn.
...
We use the `blast-target` service to demonstrate the difference: reachable before policy, blocked afterward.
...
Service `blast-target` dùng để minh hoạ: trước policy thì truy cập được, sau policy thì bị chặn.
...
We also include a lateral blast probe via `--blast-host blast-target` to show internal pod reachability.
...
You should see `BLAST_OK` in the `reply_preview` when the blast probe succeeds (before NetworkPolicy).
...
Ngoài ra có thêm probe blast với `--blast-host blast-target` để minh hoạ khả năng truy cập pod nội bộ.
...
Nếu blast thành công (trước NetworkPolicy), `reply_preview` sẽ chứa `BLAST_OK`.
...
For the lateral blast demo, `build_http_request()` creates a raw HTTP request and `send_raw_via_ssrf()` sends it via the same SSRF primitive to `blast-target`.

If you only need the blast probe without Redis poisoning, run the script with `--blast-only --blast-host blast-target`.
...
Với blast lateral, `build_http_request()` tạo HTTP thô và `send_raw_via_ssrf()` gửi qua SSRF tới `blast-target`.
